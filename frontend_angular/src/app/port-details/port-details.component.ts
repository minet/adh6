...
export class PortDetailsComponent implements OnInit, OnDestroy {
  ...
  toggleStatus() {
    this.portService.setPortStatus(this.portID, !this.portStatus)
      .subscribe((status) => {
  ...
  changeVlan(newVlan) {
    this.portService.setPortVlan(this.portID, newVlan)
      .subscribe((vlan) => {
        this.vlan = vlan;
      });
  }
  ...
  ngOnInit() {
    ...
    this.portService.getPortStatus(this.portID)
      .subscribe((status) => {
        this.setStatus(status);
      });
    this.portService.getPortVlan(this.portID)
      .subscribe((vlan) => {
        this.vlan = vlan;
      });
  }
