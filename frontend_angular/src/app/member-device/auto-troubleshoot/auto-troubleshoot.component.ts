      ...
      <div class="alert alert-danger mt-4" *ngIf="statuses.length > 0">
        ...
        <ul class="text-left">
          <li *ngFor="let status of statuses">
            <ng-container
              *ngIf="status.status === 'LOGIN_INCORRECT_WRONG_PASSWORD'"
              i18n="login incorrect wrong password"
            ...
            >
            <ng-container
              *ngIf="status.status === 'LOGIN_INCORRECT_WRONG_MAC'"
              i18n="login incorrect wrong mac"
            ...
            >
            <ng-container
              *ngIf="status.status === 'LOGIN_INCORRECT_WRONG_USER'"
              i18n="login incorrect wrong user"
              >Vous avez essayé de vous connecter avec votre appareil
              <code>{{ status.comment }}</code> mais depuis un autre compte ({{
                status.lastTimestamp | date: "short"
              }})</ng-container
            >
            <ng-container
              *ngIf="status.status === 'LOGIN_INCORRECT_SSL_ERROR'"
              i18n="login incorrect ssl error"
              >L'appareil <code>{{ status.comment }}</code> est mal configuré,
              vérifiez la configuration via les tutoriels, en particulier la
              présence de MSCHAPv2 et la non-validation du certificat ({{
                status.lastTimestamp | date: "short"
              }})</ng-container
            >
          </li>
