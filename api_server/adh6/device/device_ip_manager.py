...
class DeviceIpManager:
    def __init__(
        self, ip_allocator: IpAllocator, device_repository: DeviceRepository, vlan_manager: VlanManager
    ) -> None:
    ...
    def allocate_ips(
        ...
        devices, _ = self.device_repository.search_by(
            limit=DEFAULT_LIMIT,
            offset=DEFAULT_OFFSET,
            device_filter=DeviceFilter(member=member.id, connection_type=device_type if device_type else None),
        )
        for d in devices:
            self.allocate_ip_with_vlan(device=d, member=member, vlan=vlan)

    @log_call
    def unallocate_ips(self, member: Member) -> None:
        devices, _ = self.device_repository.search_by(
            limit=DEFAULT_LIMIT, offset=DEFAULT_OFFSET, device_filter=DeviceFilter(member=member.id)
        )
        for d in devices:
    ...
    def allocate_ip_with_vlan_number(self, device: Device, member: Member, vlan_number: int) -> None:
        ...
        self.allocate_ip_with_vlan(device=device, member=member, vlan=vlan)

    @log_call
    def allocate_ip_with_vlan(self, device: Device, member: Member, vlan: Vlan | None) -> None:
        ipv4_network = ""
    ...
    def _allocate_ip(self, device: Device, ipv4_network: str = "", ipv6_network: str = "") -> None:
        self.device_repository.update(  # type: ignore  # TODO: typing is baaaaad
    ...
    def unallocate_ip(self, device: Device) -> None:
        self.device_repository.update(
            abstract_device=AbstractDevice(  # type: ignore  # TODO: typing is baaaaad
                id=device.id, ipv4_address="En attente", ipv6_address="En attente"
            ),
            override=True,
        )
