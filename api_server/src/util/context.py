...
from src.constants import CTX_SQL_SESSION, CTX_LOGIN, CTX_TESTING, CTX_REQUEST_ID, CTX_REQUEST_URL


def build_context(ctx: MappingProxyType = None, session=None, login=None, testing=None, request_id=None, url=None):
    """
...
def log_extra(context: MappingProxyType, **extra_fields):
    admin_login = None
    if context.get(CTX_LOGIN):
        admin_login = context.get(CTX_LOGIN)
