...
def seed():
    ...
    account_types = (
        ...
        [3, "Club interne"],
        [4, "Club externe"],
        [5, "Association externe"],
    )
    session.bulk_save_objects([AccountType(id=e[0], name=e[1]) for e in account_types])

    print("Seeding MiNET accounts")
    accounts = (
        [1, 1, "MiNET frais techniques", True, None, True, True],
        [2, 1, "MiNET frais asso", True, None, True, True],
    )
    session.bulk_save_objects(
        [
            Account(id=e[0], type=e[1], name=e[2], actif=e[3], adherent_id=e[4], compte_courant=e[5], pinned=e[6])
            for e in accounts
        ]
    )
    ...
    products = (
        [1, "Cable 3m", 3, 3],
        [2, "Cable 5m", 5, 5],
        [3, "Adaptateur USB/Ethernet", 13.00, 13.00],
        [4, "Adaptateur USB-C/Ethernet", 12.00, 12.00],
    )
    session.bulk_save_objects([Product(id=e[0], name=e[1], buying_price=e[2], selling_price=e[3]) for e in products])

    print("Seeding payment methods")
    payment_methods = [1, "Liquide"], [2, "Chèque"], [3, "Carte bancaire"], [4, "Virement"], [5, "Stripe"], [6, "Aucun"]
    session.bulk_save_objects([PaymentMethod(id=e[0], name=e[1]) for e in payment_methods])

    print("Seeding cashbox")
    session.add(Caisse(fond=0, coffre=0))

    print("Seeding vlans")
    vlans = (
        [41, "157.159.41.0/24", "2001:660:3203:401::/64", "157.159.41.1/32", ""],
        [35, "10.42.0.0/16", "", "", ""],
        [36, "157.159.192.0/22", "", "157.159.195.0/24", ""],
        [30, "172.30.0.0/16", "", "", ""],
    )
    session.bulk_save_objects(
        [Vlan(numero=e[0], adresses=e[1], adressesv6=e[2], excluded_addr=e[3], excluded_addrv6=e[4]) for e in vlans]
    )
    ...
    session.bulk_save_objects(
        [Port(rcom=0, numero="1/0/" + str(i), oid="1010" + str(i), switch_id=1, chambre_id=i) for i in range(1, 30)]
    )
    print("Seeding role mapping for oidc")
    session.bulk_save_objects(
        [
            AuthenticationRoleMapping(identifier="adh6_admin", role=i, authentication=AuthenticationMethod.OIDC)
    ...
    session.bulk_save_objects(
        [
            AuthenticationRoleMapping(identifier="adh6_treso", role=i, authentication=AuthenticationMethod.OIDC)
...
def fake(login):
    ...
    for _ in range(1, 4):
        session.add(Device(mac=fake.mac_address(), ip=None, adherent_id=adherent.id, ipv6=None, type=0))
    for _ in range(1, 4):
        session.add(Device(mac=fake.mac_address(), ip=None, adherent_id=adherent.id, ipv6=None, type=1))
    session.commit()
