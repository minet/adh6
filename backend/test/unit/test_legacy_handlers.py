...
# ===========================================================================
# AccountTypeHandler tests
# ===========================================================================
class TestAccountTypeHandler:
    @fixture
    def mock_account_type_manager(self):
        from adh6.treasury.account_type_manager import AccountTypeManager

        return MagicMock(spec=AccountTypeManager)

    @fixture
    def handler(self, mock_account_type_manager):
        from adh6.treasury.http.account_type import AccountTypeHandler

        return AccountTypeHandler(account_type_manager=mock_account_type_manager)

    async def test_search(self, handler, mock_account_type_manager):
        account_type_mock = MagicMock(to_dict=lambda: {"id": 1, "name": "Adhérent"})
        mock_account_type_manager.search = AsyncMock(return_value=([account_type_mock], 1))
        result, status, headers = await unwrap(handler.search)(handler)
        assert status == 200

    async def test_get(self, handler, mock_account_type_manager):
        account_type_mock = MagicMock(to_dict=lambda: {"id": 1, "name": "Adhérent"})
        mock_account_type_manager.get_by_id = AsyncMock(return_value=account_type_mock)
        result, status = await unwrap(handler.get)(handler, id_=1)
        assert status == 200


# ===========================================================================
# TreasuryHandler tests
# ===========================================================================
class TestTreasuryHandler:
    @fixture
    def mock_cashbox_manager(self):
        from adh6.treasury.cashbox_manager import CashboxManager

        return MagicMock(spec=CashboxManager)
    ...
    @fixture
    def handler(self, mock_cashbox_manager, mock_account_manager):
        from adh6.treasury.http.treasury import TreasuryHandler

        return TreasuryHandler(cashbox_manager=mock_cashbox_manager, account_manager=mock_account_manager)

    async def test_get_cashbox(self, handler, mock_cashbox_manager):
        mock_cashbox_manager.get_cashbox = AsyncMock(return_value=(100, 200))
        result, status = await unwrap(handler.get_cashbox)(handler)
        assert status == 200
        assert result["fond"] == 100
        assert result["coffre"] == 200

    def test_get_bank(self, handler, mock_account_manager):
        mock_account_manager.get_cav_balance = MagicMock(return_value=5000)
        result, status = unwrap(handler.get_bank)(handler)
        assert status == 200
        assert result["expectedCav"] == 5000
