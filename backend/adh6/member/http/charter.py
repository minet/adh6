...
class CharterHandler:
    ...
    @with_context
    async def member_search(self, charter_id: int):
        result, total_count = await self.charter_manager.get_members(charter_id)
        headers = {"X-Total-Count": str(total_count), "access-control-expose-headers": "X-Total-Count"}
        return result, 200, headers

    @with_context
    async def member_post(self, charter_id: int, id_: int):
        if get_user() != id_ and Roles.ADMIN_READ.value not in get_roles():
            raise UnauthorizedError("Unauthorize to access this resource")
        await self.charter_manager.sign(charter_id, id_)
        return None, 201

    @with_context
    async def member_get(self, charter_id: int, id_: int):
        if get_user() != id_ and Roles.ADMIN_READ.value not in get_roles():
            raise UnauthorizedError("Unauthorize to access this resource")
        m = await self.charter_manager.get(charter_id, id_)
