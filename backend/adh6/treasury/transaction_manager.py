...
class TransactionManager(CRUDManager):
    ...
    async def update_or_create(
        ...
        transaction, created = await super().update_or_create(
            abstract_transaction, id=id
        )

        if created:
            logging.debug(
                "cashbox_update"
            )  # noqa: LOG015  # TODO: use a dedicated logger
            if transaction.cashbox == "to":
                await self.cashbox_repository.update(
                    value_modifier=transaction.value, transaction=transaction
                )
    ...
    async def partially_update(
        ...
        **kwargs
        ...
        if any(
            ...
            raise ValidationError(
                "you are trying to update a transaction with fields that cannot be updated"
            )
    ...
    async def validate(self, id: int):
        ...
        if not transaction.pending_validation:
            raise ValidationError(
                "you are trying to validate a transaction that is already validated"
            )
    ...
    async def delete(self, id: int):
        ...
        if not transaction.pending_validation:
            raise ValidationError(
                "you are trying to delete a transaction that is already validated"
            )
