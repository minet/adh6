...
class ProductManager(CRUDManager):
    ...
    async def buy(self, member_id: int, payment_method_id: int, product_ids: list[int] = []) -> None:
        ...
        dst_accounts, _ = await self.account_repository.search_by(
        ...
        src_accounts, _ = await self.account_repository.search_by(limit=1, filter_=AbstractAccount(member=member_id))
        # If the account is not found, raise an error
        if not src_accounts:
            raise AccountNotFoundError(KnownAccountExpense.TECHNICAL_EXPENSE.value)
        ...
        for i in product_ids:
            ...
            product = await self.product_repository.get_by_id(i)

            # Create a new transaction object using the transaction repository
            _ = await self.transaction_manager.update_or_create(
                AbstractTransaction(
